...
REGION_CONFIG = {
    "IND": ("https://loginbp.ggpolarbear.com", "https://client.ind.freefiremobile.com"),
    "ME": ("https://loginbp.ggpolarbear.com", "https://clientbp.ggblueshark.com"),
    "VN": ("https://loginbp.ggpolarbear.com", "https://clientbp.ggpolarbear.com"),
    "BD": ("https://loginbp.ggpolarbear.com", "https://clientbp.ggwhitehawk.com"),
    "PK": ("https://loginbp.ggpolarbear.com", "https://clientbp.ggblueshark.com"),
    "SG": ("https://loginbp.ggpolarbear.com", "https://clientbp.ggpolarbear.com"),
    "BR": ("https://loginbp.ggpolarbear.com", "https://client.us.freefiremobile.com"),
    "NA": ("https://loginbp.ggpolarbear.com", "https://client.us.freefiremobile.com"),
    "ID": ("https://loginbp.ggpolarbear.com", "https://clientbp.ggpolarbear.com"),
    "RU": ("https://loginbp.ggpolarbear.com", "https://clientbp.ggpolarbear.com"),
    "TH": ("https://loginbp.ggpolarbear.com", "https://clientbp.ggpolarbear.com"),
}
...
def perform_major_login(access_token, open_id, login_base_url):
    major_login_url = f"{login_base_url}/MajorLogin"
    platforms = [8, 3, 4, 6]
...
def upload_bio_request(jwt_token, bio_text, server_base_url):
    update_url = f"{server_base_url}/UpdateSocialBasicInfo"
    try:
        data = BioData()
        data.field_2 = 17
...
def index():
    return jsonify({
        "message": "🔥 Free Fire Bio Changer API 🔥",
        "credit": "RAJ GMR",
        "telegram": "@RAJGAMER",
        "channel": "https://t.me/RAJ_GAMER_01",
        "endpoint": "/bio_upload",
        "methods": ["GET", "POST"],
        "required_parameters": {
            "region": "Server region code (IND, ME, VN, BD, PK, SG, BR, NA, ID, RU, TH)",
            "bio": "New bio text (max length varies per game)"
        },
        "auth_methods": {
            "jwt": "Direct JWT token (parameter: jwt)",
            "uid_pass": "UID + password (parameters: uid, pass)",
            "access_token": "Access token (parameter: access)"
        },
        "example_requests": [
            "/bio_upload?region=BR&bio=Hello&uid=123456&pass=xxx",
            "/bio_upload?region=IND&bio=MyBio&access=YOUR_ACCESS_TOKEN",
            "/bio_upload?region=ME&bio=Test&jwt=eyJhbGciOiJIUzI1NiIs..."
        ],
        "all_regions": list(REGION_CONFIG.keys())
    })

# ---------- Main Bio Upload Endpoint ----------
...
def combined_bio_upload():
    ...
    region = (request.args.get("region") or request.form.get("region") or "").upper()

    if not bio:
        return jsonify({"status": "❌ Error", "code": 400, "error": "Missing 'bio' parameter"}), 400

    if not region:
        return jsonify({"status": "❌ Error", "code": 400, "error": "Missing 'region' parameter. Use one of: " + ", ".join(REGION_CONFIG.keys())}), 400

    if region not in REGION_CONFIG:
        return jsonify({"status": "❌ Error", "code": 400, "error": f"Unknown region '{region}'. Allowed: {', '.join(REGION_CONFIG.keys())}"}), 400

    login_base_url, server_base_url = REGION_CONFIG[region]
    ...
    if jwt_token:
        ...
        if acc_token and login_openid:
            ...
            final_jwt = perform_major_login(final_access_token, final_open_id, login_base_url)
            if final_jwt:
        ...
        if final_open_id:
            final_jwt = perform_major_login(access_token, final_open_id, login_base_url)
        else:
            return jsonify({"status": "❌ Shop2Game OpenID Fetch Failed", "code": 400}), 400

    else:
        return jsonify({"status": "❌ Error", "code": 400, "error": "Provide JWT, or UID/Pass, or Access Token, and region"}), 400

    if not final_jwt:
        return jsonify({"status": "❌ JWT Generation Failed", "code": 500}), 500

    result = upload_bio_request(final_jwt, bio, server_base_url)

    response_data = {
        "Credit": "RAJ GMR",
        "Join For More": "Telegram: @RAJGAMER01",
        "Telegram_Channel": "https://t.me/RAJ_GAMER01",
...
if __name__ == "__main__":
    app.run(host="127.0.0.1", port=5990, debug=False)